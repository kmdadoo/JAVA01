	...
	public static void bubbleSort(int[] arrParam, int ordSel)
	{
		...
		*/
		for (int i = 0; i < arrParam.length-1; i++)
		{
			/*
				실제 요소값에 대한 비교를 진행한다.
				버블정렬의 목적은 오름차순일때 큰 숫자를 제일 뒤로 보내주는
...
//						showArrary(arrParam, "SWAP중(오름차순 정렬중):"+
//											(swapCount++) + "회 교환됨");
					}
				}else if(ordSel==2) // 내림차순을 선택했을 때
				{
					if(arrParam[j] < arrParam[j+1])
					{
						...
						*/
						...
						// 값의 변경이 있을때마다 배열전체 출력하기
//						showArrary(arrParam, "SWAP중(내림차순 정렬중):"+
//											(swapCount++) + "회 교환됨");
					}
				}
			}
	...
	public static void showArrary(int[] arr, String message)
	{
	...
	public static void randomCreate(int[] arrParam)
	{
		...
		*/
		while(true)
		{
			...
			{
				arrParam[i] = rnd.nextInt(45) + 1; // 1~45까지의 정수 생성
			}
			
			// 2.중복 확인
			boolean rndFlag = false;/* 중복 체크를 위한 변수(false라면 중복된
				난수가 없는 경우이고, 중복된 난수가 발견된다면 true로 값을
				변경할 것임. */
			...
			for (int i = 0; i < arrParam.length; i++)
			{
				for (int j = 0; j < arrParam.length; j++)
				{
					/*
					 	...
					 	인덱스가 동일하다면 같은 숫자일테니끼...
					*/
					if(i!=j && arrParam[i]==arrParam[j])
					{
						// 중복된 값이 발견되다면 true로 값을 변경
						rndFlag = true;
					}
				}
			}
