...
public class InsertSQL extends IConnectImpl 
{
	...
	public void execute() {
		try {
			// 1. 쿼리문 준비 : 값의 세팅이 필요한 부분을 ?(인파라미터)로
			// 기술한다.
			String query = "INSERT INTO member VALUES (?, ?, ?, ?)";
			
			// 2. prepared객체생성 : 객체 생성시 준비한 쿼리문을
			// 인수로 전달한다.
			...
			/*
			4. 인파라미터 설정 : ? 의 순서대로 설정하고, 인덱스는 
				1부터 시작한다.
				자료형이
					숫자면 setInt()
					문자면 setString()
					날자면 setDate()
				입력값이 문자 혹은 날짜면 자동으로 싱글쿼테이션이 
				추가된다.
			*/
			psmt.setString(1, id);
			psmt.setString(2, pw);
...
//			psmt.setString(4, "2022-10-27");
			...
			// 날자 입력2 : Date형으로 입력하는 경우
			/*
			현재날짜를 Java에서 입력하는 경우 아래와 같이 변환과정을
			거쳐야 한다.  Util패키지로 시간을 얻어온 후 sql패키지로 타입을
